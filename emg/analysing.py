...
class AnalysingResults:
    ...
    def _analysis(self, grouped_var, predictor, plot=False, **kwargs):
        ...
        if plot:
            ...
            if kwargs['ax'].get_subplotspec().is_first_col():
                kwargs['ax'].set_ylabel(kwargs['y_label'])
            else:
...
class AnalysisContainer:
    ...
    def df_with_diffrent_columns(self):
        ...
        for title, analysis in self.container.items():
            ...
            for key in keys_copy:
                if key in ['pp', 'training', 'set', 'type_analysis']:
                    ...
                    new_dict[f"{key}_{title.split('_')[3]}"] = analysis.results[key]
        ...
        #merged_df = merged_df.groupby(['training', 'pp', 'set']).sum().reset_index()
        #print(merged_df.head(10))
        return merged_df
